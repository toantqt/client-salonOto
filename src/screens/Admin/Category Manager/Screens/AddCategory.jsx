...
import { addCategory, addService } from "../../../../api/AdminAPI";
  ...
  const handleSubmit = async () => {
    ...
    if (data.name === "") {
      ...
      await addCategory(data).then(() => {
        history.push(`${AdminSlug.categoryManager}?q=${type}`);
      });
    }
    console.log(data);
  };
  ...
  return (
    <Grid>
      <div className="header-title mb-3">
        <span>Thêm {type === "company" ? "hãng xe" : "loại xe"} mới: </span>
      </div>
      <div>
        <Grid container spacing={2}>
          <Grid item lg={12}>
            <TextField
              id="outlined-basic"
              label={"Tên " + (type === "company" ? "hãng xe" : "loại xe")}
              variant="outlined"
              name="name"
              style={{ width: "100%" }}
